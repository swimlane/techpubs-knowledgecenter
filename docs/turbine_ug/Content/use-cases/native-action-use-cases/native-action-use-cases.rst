Native Action Use Cases
=======================

This section of the guide provides specific native action use cases in
Swimlane Turbine.

A use case is a succinct story of how to successfully complete a
scenario within Turbine.

.. toctree::
   :titlesonly:
   :caption: Children:

   /Content/use-cases/native-action-use-cases/transform-data-date-time-examples
   /Content/use-cases/native-action-use-cases/http-requests
   /Content/use-cases/native-action-use-cases/script-test-use-case
   /Content/use-cases/native-action-use-cases/variables-use-case
   /Content/use-cases/native-action-use-cases/loop-use-case
   /Content/use-cases/native-action-use-cases/ifelse-use-case
   /Content/use-cases/native-action-use-cases/nesting-branching-and-conditions-use-case
   /Content/use-cases/native-action-use-cases/get-gmail-emails-list-use-case
   /Content/use-cases/native-action-use-cases/get-attachment-ids-as-a-file-use-case
